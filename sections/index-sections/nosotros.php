<div id="nosotros" class="row featurette pb-6">

    <div class="col-md-5 img-nos-container">
        <img src="css\img\page-img\img-estudios.png" alt="img-estudios">
    </div>

    <div class="col-md-6 my-auto info-nos">
        <span class="decorators">NOSOTROS</span>
        <h2 class="mb-4">Hidronaval es una organización de <span class="text-muted">capacitacion.</span></h2>
        <h4 class="mb-4">Técnicos que capacitan a mas técnicos</h4>
        <p class="mb-4">Hidronaval  te ofrece una formación de calidad vía e-learning. Los programas de perfeccionamiento profesional están en crecimiento y al respaldo de los trabajadores para que aumenten su capacitación y se adapten a los nuevos requerimientos productivos.</p>
        <a href="#cursos" class="btn hidro-buttons">Ver cursos</a>
    </div>

</div>